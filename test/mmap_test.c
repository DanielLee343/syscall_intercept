...
long nearestMultipleOf4096(long num)
{
  long remainder = num % 4096;
  if (remainder >= 2048)
    return num + (4096 - remainder);
  else
...
int calculateIntersectedArea_(long int A, long int B, long int C, long int D, long int *start, long int *end)
{
  ...
  if (C < A && A < D && D < B)
  { // case 1: need to align D
    printf("1st case\n");
    *start = A; // A must be aligned already
    *end = align_to_page(D);
  }
  else if (A < C && C < D && D < B)
  { // case 2: need to align both C and D
    printf("2nd case\n");
    *start = align_to_page(C);
    *end = align_to_page(D);
  }
  else if (A < C && C < B && B < D)
  { // case 3: need to align C
    printf("3rd case\n");
    *start = align_to_page(C);
